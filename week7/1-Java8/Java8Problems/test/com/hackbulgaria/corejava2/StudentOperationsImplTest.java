...
public class StudentOperationsImplTest {
	...
	@Before
	public void setup() throws InitializationError, InstantiationException,
			IllegalAccessException, IllegalArgumentException,
			InvocationTargetException, NoSuchMethodException, SecurityException {
		Reflections reflections = new Reflections(getClass().getPackage()
				.getName());
		Set<Class<? extends StudentOperations>> subtypes = reflections
				.getSubTypesOf(StudentOperations.class);
		if (subtypes.isEmpty()) {
			throw new InitializationError("You have not subclassed"
					+ StudentOperations.class.getCanonicalName());
		}
		if (subtypes.size() > 1) {
			throw new InitializationError("One subclass of "
					+ StudentOperations.class.getCanonicalName()
					+ " will do, No need for more.");
		}
		this.allStudents = StudentsDataFactory.getAllStudents();
		this.studentOperations = subtypes.iterator().next()
				.getConstructor(java.util.List.class).newInstance(allStudents);
	}

	@Test
	public void testGetStudentsWithLowestMarks() throws Exception {
		java.util.List<Student> students = studentOperations
				.getStudentsWithLowestMarks();
	...
	public void testGetStudentsWIthHighestMarks() throws Exception {
		List<Student> students = studentOperations
				.getStudentsWithHighestMarks();
	...
	public void testSplitStudentsByMarks() {
		Map<Boolean, List<Student>> map = studentOperations
				.splitStudentsByMarks(3.5f);
	...
	public void testDistributeByAge() {
		Map<Integer, List<Double>> gradesByAge = studentOperations
				.getMarksDistributionByAge();
		assertTrue(gradesByAge.get(20).stream().reduce(Double::sum).get()
				.compareTo(15.0) == 0);
	}
	...
	public void testGetAverageMarkByGender() {
		...
		double maleAvg = allStudents.stream()
	...
	public void testDistributeMarks() {
		Map<Double, Integer> distribution = studentOperations
	...
	public void testGetEmailToHeader() {
		...
		assertEquals(expectedResult.replaceAll("\\s+", "").toLowerCase(),
				studentOperations.getEmailToHeader().replaceAll("\\s+", "")
						.toLowerCase());
	}
	...
	public void testSplitStudentMarksByGenderAndThenByAge() {
		Map<Gender, Map<Integer, List<Student>>> map = studentOperations
				.splitStudentMarksByGenderAndThenByAge();
